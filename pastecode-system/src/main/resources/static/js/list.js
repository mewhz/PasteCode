...
$(document).ready(()=>{
    ...
    $.ajax({
        ...
        success: (result) => {
            ...
            for (let i = 0; i < len; i ++){
                all.append("<div><details>" +
                    "<summary>"+ date[i] +" " + time_id[i] + " " + remark[i] + "</summary>" +
                    "<pre><code class='language-"+ type[i] +"' id='code-" + i + "'>"+ text[i] +"</code></pre>" +
                    "<button class=\"btn btn-success button\" data-clipboard-target=\"#code-" + i + '\" data-clipboard-action=\"copy\">一键复制</button>' +
                    "</details></div>");
                if (!set.has(type[i])){
                    switch (type[i]){
                        case "java":
